...
```
git clone https://github.gatech.edu/gios-fall-25/pr3
```

We encourage you to save your code in a separate repository. You can make one in the **gios-fall-25**
...
cache to the proxy process by way of shared memory. There is no need
to send a curl request to a server in case of a cache miss. The proxy can simply return
GF_FILE_NOT_FOUND if the file is not available.
...
In the situation where multiple clients (proxy servers) are attached to the cache, 
...
The cache must set up some command channel/communication mechanism (socket, message
queue, shared memory) by which the proxy can communicate its request along with the information
about the communication mechanism (shared memory name, id, etc.). This would include cleaning
up the command channel if it's no longer being used. For the purposes of this project, this
mechanism does not have to be robust to misbehaving clients.
...
- **webproxy.c** - (modify and submit) this is the main file for the webproxy program. Only small modifications are necessary. In addition to setting up the callbacks for the gfserver library, you must pre-create a pool of shared memory descriptors, a queue of free shared memory descriptors, associated synchronization variables, etc. The segment count and segment size parameters should be used.
